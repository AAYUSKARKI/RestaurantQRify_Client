...
import type { Bill, BillState, CreateBillRequest } from "@/types/Bill";
...
export const fetchBills = createAsyncThunk<Bill[], void, { rejectValue: string }>(
    ...
    async (_, { rejectWithValue }) => {
        try {
            const response = await api.get<{ data: Bill[] }>("/bill");
...
export const createBill = createAsyncThunk<Bill, CreateBillRequest, { rejectValue: string }>(
    "bill/create",
    async (data, { rejectWithValue }) => {
        try {
            const response = await api.post<{ data: Bill; message: string }>("/bill", data);
...
export const payBill = createAsyncThunk<Bill, string, { rejectValue: string }>(
    "bill/pay",
    async (id, { rejectWithValue }) => {
        try {
            const response = await api.post<{ data: Bill }>(`/bill/${id}/pay`);
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(payBill.fulfilled, (state, action) => {
                ...
                if (state.selectedBill?.id === action.payload.id) state.selectedBill = action.payload;
            })
