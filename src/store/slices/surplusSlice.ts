...
export const fetchDailySpecials = createAsyncThunk<DailySpecial[], void, { rejectValue: string }>(
    "surplus/fetchDaily",
    async (_, { rejectWithValue }) => {
        try {
            const response = await api.get<{ data: DailySpecial[] }>("/surplus");
...
export const createSurplusMark = createAsyncThunk<SurplusMark, CreateSurplusRequest, { rejectValue: string }>(
    "surplus/create",
    async (data, { rejectWithValue }) => {
        try {
            const response = await api.post<{ data: SurplusMark; message: string }>("/surplus", data);
...
export const updateSurplusMark = createAsyncThunk<SurplusMark, { id: string; data: UpdateSurplusRequest }, { rejectValue: string }>(
    "surplus/update",
    async ({ id, data }, { rejectWithValue }) => {
        try {
            const response = await api.put<{ data: SurplusMark }>(`/surplus/${id}`, data);
...
export const deleteSurplusMark = createAsyncThunk<string, string, { rejectValue: string }>(
    "surplus/delete",
